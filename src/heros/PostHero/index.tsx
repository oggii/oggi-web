...
export const PostHero: React.FC<Props> = ({ post }) => {
  ...
  const authorNames = populatedAuthors?.map((a) => a.name).filter(Boolean).join(', ')
  ...
  return (
    <div className="relative min-h-[60vh] flex items-end">
      <div className="max-w-4xl mx-auto px-6 z-10 relative pb-12 w-full">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-medium tracking-tight text-white mb-6">{title}</h1>
      ...
      {heroImage && typeof heroImage === 'object' && heroImage.url && (
        <div className="absolute inset-0">
          <Image src={heroImage.url} alt={heroImage.alt || title} fill priority className="object-cover" />
