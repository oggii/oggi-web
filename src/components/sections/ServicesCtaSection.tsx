...
export default function ServicesCtaSection() {
  ...
  return (
    <section className="relative py-24 px-6 flex flex-col items-center justify-center text-center bg-luxota-bg border-t border-white/5 overflow-hidden">
      ...
      {/* Label */}
      <span className="text-xs text-luxota-accent font-mono mb-6 tracking-widest">{t('cta.tag')}</span>

      {/* Heading */}
      <h2 className="text-3xl md:text-5xl font-medium text-white tracking-tight mb-4 leading-tight max-w-2xl">
        {t('cta.title')}
      </h2>
      <p className="text-luxota-dim text-base md:text-lg font-light max-w-xl mb-12 leading-relaxed">
        {t('cta.desc')}
      </p>
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-center gap-4 text-sm text-luxota-dim">
        <Link href="/openclaw" className="hover:text-white transition-colors flex items-center gap-1.5 group">
          <span className="w-1.5 h-1.5 rounded-full bg-luxota-accent/60 group-hover:bg-luxota-accent transition-colors" />
          {t('cta.link1')}
        </Link>
        <span className="hidden sm:block text-white/10">·</span>
        <Link href="/hermes" className="hover:text-white transition-colors flex items-center gap-1.5 group">
          <span className="w-1.5 h-1.5 rounded-full bg-luxota-accent/60 group-hover:bg-luxota-accent transition-colors" />
          {t('cta.link2')}
        </Link>
        <span className="hidden sm:block text-white/10">·</span>
        <Link href="/#action" className="hover:text-white transition-colors flex items-center gap-1.5 group">
          <span className="w-1.5 h-1.5 rounded-full bg-luxota-accent/60 group-hover:bg-luxota-accent transition-colors" />
          {t('cta.link3')}
        </Link>
      </div>
