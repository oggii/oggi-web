...
export const FormBlock: React.FC<FormBlockProps> = ({ enableIntro, form, introContent }) => {
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        const dataToSend = Object.entries(formData).map(([field, value]) => ({ field, value }))
        ...
        {hasSubmitted && form?.confirmationType === 'message' && form.confirmationMessage ? (
          ...
          <form onSubmit={onSubmit}>
                  ...
                  return (
                    <div key={index}>
                      ...
                      <textarea
                        ...
                        rows={4}
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder:text-white/30 focus:border-white/30 focus:outline-none transition-colors"
                        value={formData[name] || ''}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="mt-6 bg-white text-black px-6 py-2.5 rounded-lg font-medium text-sm hover:bg-white/90 transition-colors disabled:opacity-50"
            >
