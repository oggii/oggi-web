...
export const ArchiveBlock: React.FC<Props> = async (props) => {
  ...
  if (populateBy === 'collection') {
    ...
    const flatCats = categories?.map((c) => (typeof c === 'object' ? c.id : c))

    const fetched = await payload.find({
      collection: 'posts',
      depth: 1,
      limit,
      ...(flatCats?.length ? { where: { categories: { in: flatCats } } } : {}),
    })
    posts = fetched.docs
  } else if (selectedDocs?.length) {
    posts = selectedDocs.map((d) => (typeof d.value === 'object' ? d.value : null)).filter(Boolean)
  }
          ...
          >
            {post.featuredImage?.url && (
              <div className="relative aspect-video overflow-hidden">
                <Image src={post.featuredImage.url} alt={post.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
            )}
            <div className="p-5">
              <h3 className="text-white font-medium text-lg mb-2">{post.title}</h3>
              {post.excerpt && <p className="text-white/50 text-sm line-clamp-2">{post.excerpt}</p>}
            </div>
          </Link>
        ))}
      </div>
